 ...
 */
define(['lib/util','EventUtil'], function( util, EventUtil ) {
	...
	dialogueFromMDD.prototype.createForm = function( form ) {
		...
		util.forEach(form.inputs, function(input, index) {
			...
			// text area
			else if ( input.type && input.type == 'textarea' ) {				
				var element = util.createElement({
					tag : 'textArea',
					attributes : {
			...
			// if it's not a select element it's an input.
				...
				EventUtil.addEventListener( element, 'keypress', function ( event ) {
					var event = event || window.event
					var key = EventUtil.keyCode( event )

					if ( key === 13 ) {
						console.log("Enter key is disabled. Click 'Apply' button instead.")
